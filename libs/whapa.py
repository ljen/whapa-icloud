...
TYPE_FLAGS = {
    ...
    "type_call": "call",
    "type_application": "document",
    "type_gif": "gif",
    "type_deleted": "deleted",
    "type_share": "live_location",
    "type_stickers": "sticker",
    "type_system": "system",
    "type_poll": "poll",
    "type_viewonce": "view_once_image",
    "type_videonote": "video_note",
    "type_event": "event",
}
...
def build_filter(args):
    ...
    return report.Filter(
        ...
        chat=chat,
        sender=args.user_all or args.sender,
        date_from=parse_time(args.time_start),
        date_to=parse_time(args.time_end),
        ...
        only_forwarded=args.forwarded,
        only_edited=args.edited,
...
def _aviso_media(args, r):
    ...
    if args.maps and (r.get("maps_ok") or r.get("maps_failed")):
        print(
            "    Mapas: {} descargados, {} fallidos".format(
                r.get("maps_ok", 0), r.get("maps_failed", 0)
            )
        )
    if faltan and not hallados:
        print(
            Fore.YELLOW + "    [!] No se ha localizado ningun adjunto. Comprueba "
...
def mode_messages(ext, flt, args, out_dir):
    ...
    for c in chats:
        ...
        if args.web:
            msgs = [m for m in msgs if reader.infer_device(m.key_id) == "WhatsApp Web"]
        if msgs:
    ...
    print(f"[i] {total} mensaje(s) en {len(seleccion)} chat(s)")

    if not (args.report or args.print_report or args.csv):
        for c, msgs in seleccion:
            print("\n" + Fore.CYAN + f"=== {c.label} ({c.chat_id}) ===" + Fore.RESET)
            for m in msgs[: args.limit]:
                quien = "Yo" if m.from_me else (m.sender_label or c.label)
                print(
                    "  [{}] {}{}: {}".format(
                        reader.fmt_ts(m.timestamp),
                        quien,
                        "" if m.kind is codes.Kind.TEXT else f" <{m.type_desc}>",
                        (m.display_text or "").replace("\n", " ")[:160],
                    )
                )
            if len(msgs) > args.limit:
                print(f"  ... y {len(msgs) - args.limit} mas (usa -r, -p o -x)")
    return seleccion
...
def mode_info(ext, opt, args):
    ...
    if opt == "1":
        ...
        for m in msgs[: args.limit]:
            print(
                "  [{}] {} : {}".format(
                    reader.fmt_ts(m.timestamp), m.sender or "", m.display_text[:120]
                )
            )
        ...
        for c in calls[: args.limit]:
            print(
                "  [{}] {} {} {} {}".format(
                    reader.fmt_ts(c.timestamp),
                    reader.short_jid(c.jid),
        ...
        if not calls:
            print(
                Fore.YELLOW + "  (sin registro de llamadas en esta base)" + Fore.RESET
            )
        ...
        if not sin_catalogar:
            print(
                Fore.GREEN
                + "  Todos los tipos presentes estan catalogados."
                + Fore.RESET
            )
            return
        for t in sorted(sin_catalogar):
            ...
            print(
                Fore.CYAN + f"  Codigo {t}" + Fore.RESET + f"  ({len(msgs)} mensaje(s))"
            )
            print(
                f"    con texto {con_texto} | con archivo {con_media} | con coordenadas {con_coord} | "
                f"de sistema {del_sistema} | enviados por el usuario {enviados}"
            )
            for x in msgs[:3]:
                print(
                    "      [{}] {} -> {}".format(
                        ...
                        (
                            x.text
                            or x.media_path
                            or "(sin contenido en la tabla message)"
                        )[:70].replace("\n", " "),
                    )
                )
            print()
...
def mode_extract(ext, out_dir):
    ...
    print(f"[i] {len(con_ruta)} mensaje(s) con archivo referenciado")
    copiados = faltan = 0
    ...
    with open(listado, "w", encoding="utf-8") as fh:
        ...
        for m in con_ruta:
            ...
            if origen and os.path.isfile(origen):
                try:
                    shutil.copy2(
                        origen, os.path.join(destino, os.path.basename(origen))
                    )
                    estado = "copiado"
                    copiados += 1
                except OSError as e:
    ...
    print(f"[-] {copiados} archivo(s) copiado(s), {faltan} no localizado(s)")
    print(f"[-] Indice: {listado}")
    print(
        Fore.YELLOW + "[i] La base guarda rutas, no los archivos. Los que no se "
        "localizan hay que aportarlos desde el volcado del terminal." + Fore.RESET
    )
...
def mode_carving(db_path, out_dir):
    ...
    if not os.path.exists(ruta):
        print(Fore.RED + f"[e] No se encuentra {binario} en libs/" + Fore.RESET)
        return
    salida = os.path.join(out_dir, "carving.txt")
    try:
        with open(salida, "w", encoding="utf-8", errors="replace") as fh:
            subprocess.run(
                [ruta, "--file", db_path, "--freespace"],
                stdout=fh,
                stderr=subprocess.DEVNULL,
                check=False,
            )
        print(f"[-] Carving guardado en {salida}")
    except OSError as e:
        print(Fore.RED + f"[e] No se pudo ejecutar undark: {e}" + Fore.RESET)
...
def main():
  ...
  iOS (se autodetecta):
    ...
    user_parser.add_argument(
        "-a", "--all", help="Show all chat messages", action="store_true"
    )

    parser.add_argument(
        "-wa",
        "--wa_file",
        help="Show names along with numbers (wa.db / ContactsV2.sqlite)",
    )
    ...
    parser.add_argument(
        "--platform",
        choices=[codes.ANDROID, codes.ANDROID_LEGACY, codes.IOS],
    ...
    out_parser.add_argument(
        "-x", "--csv", action="store_true", help="Export the selected messages to CSV"
    )
    out_parser.add_argument(
        "-k",
        "--kml",
    ...
    f_parser.add_argument(
        "-cs", "--case_sensitive", action="store_true", help="Case sensitive text match"
    )
    f_parser.add_argument(
        "-ww", "--whole_word", action="store_true", help="Match whole words only"
    )
    f_parser.add_argument("-sn", "--sender", help="Show messages by sender")
    f_parser.add_argument(
        "-d",
        "--direction",
        choices=["sent", "received", "system"],
    ...
    f_parser.add_argument(
        "-fw", "--forwarded", help="Show forwarded messages", action="store_true"
    )
    f_parser.add_argument(
        "-ed", "--edited", help="Show edited messages", action="store_true"
    )
    f_parser.add_argument(
        "-md", "--only_media", help="Show messages with attachment", action="store_true"
    )
    f_parser.add_argument(
        "-gp",
        "--only_location",
        help="Show messages with coordinates",
        action="store_true",
    )
    f_parser.add_argument(
        "-lr",
    ...
    f_parser.add_argument(
        "-rt", "--raw_types", help="Native type codes, comma separated (ej. 66,112)"
    )
    ...
    t_parser.add_argument(
        "-tn", "--type_poll", help="Show poll messages (new)", action="store_true"
    )
    t_parser.add_argument(
        "-tq",
        "--type_viewonce",
        help="Show view once messages (new)",
        action="store_true",
    )
    t_parser.add_argument(
        "-tj",
        "--type_videonote",
        help="Show video note messages (new)",
        action="store_true",
    )
    t_parser.add_argument(
        "-tz", "--type_event", help="Show event messages (new)", action="store_true"
    )
    ...
    if not (args.messages or args.info or args.extract or args.carving):
        sys.exit(
            "[e] Elige un modo: -m (mensajes), -i (informacion), "
    ...
    if not out_dir:
        ...
        print(f"[i] Sin -o: el informe se guardara en {out_dir}")
    ...
    s = ext.summary()
    print(
        "[i] {} mensajes - {} chats - {} contactos".format(
    ...
    if getattr(ext, "damaged_text", 0):
        print(
            Fore.YELLOW
            + f"[!] {ext.damaged_text} texto(s) con bytes no validos en el origen. "
    ...
    if args.report:
        ...
        if r["mode"] == "folder":
            print(
                "    HTML inicial {:.1f} KB - datos {:.1f} MB en {} paginas".format(
    ...
    if args.print_report:
        ...
        print(
            "[-] Informe imprimible: {} ({} mensajes, ~{} hojas A4)".format(
        ...
        if r["truncated"]:
            print(
                Fore.YELLOW + "[!] Documento truncado: afina los filtros." + Fore.RESET
            )
    ...
    if args.kml:
        ...
        if n:
            print(f"[-] KML exportado: {destino} ({n} ubicacion(es))")
        else:
            os.remove(destino)
            print(
                Fore.YELLOW + "[!] Ninguna ubicacion cumple los criterios; "
