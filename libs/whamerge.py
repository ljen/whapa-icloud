    ...
    if os.path.isdir(db_path):
        ...
        for filename in list_dbs:
            if (filename != list_dbs[0]) and (filename != db_name):
                ...
                ids_message_write_set = set(ids_message_write)
                ids_message_insert = [str(item[0]) for item in ids_message_read if item not in ids_message_write_set]
                print("   [-] New messages", len(ids_message_insert))

                print("[+] Looking for new chats")
                ids_chatlist_write_set = set(ids_chatlist_write)
                ids_chatlist_insert = [str(item[0]) for item in ids_chatlist_read if item not in ids_chatlist_write_set]
                ...
                ids_thumb_write_set = set(ids_thumb_write)
                ids_thumb_insert = [str(item[0]) for item in ids_thumb_read if item not in ids_thumb_write_set]
