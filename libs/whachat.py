...
def getDataPointiOS(line):
    ...
    splitLine = line.split(
        "] ", 1
    )  # splitLine = '[25/8/20, 10:02:14', 'Jordi Subinspector Tecnologicos: Por qué no vieron los maniquiey']
    dateTime = splitLine[0].replace("[", "")  # dateTime = '25/8/20, 10:02:14'
    try:
        date, time = dateTime.split(
            ", "
        )  # date = '25/8/20'; time = '10:02:14' # English mobile
    except Exception:
        date, time = dateTime.split(
            " "
        )  # date = '25/8/20'; time = '10:02:14' # Spanish mobile

    message = " ".join(
        splitLine[1:]
    )  # message = 'Jordi Subinspector Tecnologicos: Por qué no vieron los maniquiey'
    if startsWithAuthor(message):  # True
        splitMessage = message.split(
            ": ", 1
        )  # splitMessage = ['Jordi Subinspector Tecnologicos', 'Por qué no vieron los maniquiey']
        author = splitMessage[0]  # author = 'Jordi Subinspector Tecnologicos'
        message = " ".join(
            splitMessage[1:]
        )  # message = 'Por qué no vieron los maniquiey'
    else:
        author = None
...
def getDataPointAndroid(line):
    ...
    splitLine = line.split(
        " - "
    )  # splitLine = ['23/5/18 15:24', 'Sergio F: No se tío no le preguntao al final']
    dateTime = splitLine[
        0
    ]  # dateTime = '23/5/18 15:24'                       / 24.07.21, 10:15
    try:
        date, time = dateTime.split(
            ", "
        )  # date = '23/5/18'; time = '15:24' #  Unknow mobile
    except Exception:
        date, time = dateTime.split(
            " "
        )  # date = '23/5/18'; time = '15:24' # English mobile

    message = " ".join(
        splitLine[1:]
    )  # message = 'Sergio F: No se tío no le preguntao al final'
    if startsWithAuthor(message):  # True
        splitMessage = message.split(
            ": "
        )  # splitMessage = ['Sergio F', 'No se tío no le preguntao al final']
        author = splitMessage[0]  # author = 'Sergio F'
        message = " ".join(
            splitMessage[1:]
        )  # message = 'No se tío no le preguntao al final'
    else:
        author = None
    return date, time, author, message
        ...
        if (report_var == "EN") or (report_var == "ES"):
            try:
                message = (
                    ...
                    + "</br><iframe width='300' height='150' id='gmap_canvas' src='https://maps.google.com/maps?q={}%2C{}&t=&z=15&ie=UTF8&iwloc=&output=embed' frameborder='0' scrolling='no' marginheight='0' marginwidth='0'></iframe>".format(
                        lon, lat
                    )
                )
            except Exception:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f", "--format", help='Type a date-time mask "%d/%m/%y %H:%M:%S"', nargs="?"
    )
    parser.add_argument(
