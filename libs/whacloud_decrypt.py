...
def lzfse_decode(src, hint):
    if _lib is None:
        raise RuntimeError(
            "libcompression.dylib not found. This script requires macOS."
        )
    cap = max(hint, len(src)) * 2 + 4096
    while True:
        dst = ctypes.create_string_buffer(cap)
        got = _lib.compression_decode_buffer(
            dst, cap, src, len(src), None, COMPRESSION_LZFSE
        )
        if got == 0:
...
def decrypt_media_tar(path, out_dir, backup_key):
    ...
    print(
        f"[+] Decrypted {os.path.basename(path)} -> extracted {extracted} files, {linked} copies"
    )
...
""",
    ...
    parser.add_argument(
        "-k", "--key", required=True, help="64-character hex recovery key"
    )
    parser.add_argument(
        "-o",
        "--output",
        default="whacloud_output",
        help="Output directory (default: whacloud_output)",
    )
    parser.add_argument(
        "-b",
        "--backup-path",
        help="Path to the backup directory (if not auto-detected)",
    )
    ...
    if args.backup_path:
        ...
        if not backups:
            ...
            print(
                "    Please ensure iCloud Drive is synced and try providing --backup-path."
            )
            sys.exit(1)
