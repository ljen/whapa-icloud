...
def test_salida_por_defecto():
    ...
    r = subprocess.run([sys.executable, os.path.join(libs, "whapa.py"), db,
                        "-m", "-a", "-r", "ES"],
                       cwd=libs, capture_output=True, text=True,
...
def test_remitente_en_grupo():
    ...
    con.execute("INSERT INTO wa_contacts VALUES "
...
def test_texto_danado():
    ...
    report.export_csv(report.search(ext, report.Filter()),
...
if __name__ == "__main__":
    ...
    test_consola_segura(); test_lid(); test_nombre_de_grupo()
    test_salida_por_defecto(); test_nombres_de_contacto()
    test_remitente_en_grupo(); test_texto_danado()
